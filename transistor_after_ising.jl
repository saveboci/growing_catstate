...
println("We take L = $(L), bond dim = $(bondD), dt = $(dt). We first evolve the state allup with an TFIC hamiltonian with overall constant $(J_ising) and external field $(h_ising) for a time $(timeSTART). Then we evolve with the quantum transistor with Jx = $(Jx), Jy = $(Jy), Jz = $(Jz), h = $(h) for $(timeMAX) timeunits.")
...
function TEBD_via_Ising(psi, cutoff, bond_dim_max, timestep, ttotal, mpo = false)
    ...
    for j=1:LL-2
        ...
        Bj = - 2 * J_ising * h_ising * op("Sz",s1) * op("Id",s2) 
        hj = Aj + Bj 
    ...
    B_end = - 2 * J_ising * h_ising * op("Sz",s1) * op("Id",s2) - 2 * J_ising * h_ising * op("Id",s1) * op("Sz",s2) 
    h_end = A_end + B_end
...
for i in 1:timeSTEPS
    ...
    save("/home/sbocini/Numerics_LabDesktop/quantum_transistor/after_ising/L$(L)_Jx$(Jx)_Jy$(Jy)_Jz$(Jz)_h$(h)_Deltat$(timeSTART)_hising$(h_ising)_tTOT$(timeMAX)_BondDim$(bondD)_dt$(dt).jld2"
    #save("/Users/saveriobocini/numerics_PhD/quantum_transistor/after_ising/L$(L)_Jx$(Jx)_Jy$(Jy)_Jz$(Jz)_h$(h)_Deltat$(timeSTART)_hising$(h_ising)_tTOT$(timeMAX)_BondDim$(bondD)_dt$(dt).jld2"
        , "times", times
        , "t$(lege[1])", evolved[1]
        , "t$(lege[2])", evolved[2]
