...
#deskorlap = "/Users/saveriobocini/numerics_PhD" # for laptop
deskorlap = "/home/sbocini/Numerics_LabDesktop" # for desktop

println("We take L = $(L), bond dim = $(bondD), dt = $(dt). We first evolve the state allup with an TFIC hamiltonian with overall constant 1 and external fields hz=$(hz_ising) and hx=$(hx_ising) for a time $(Δt). Then, without flipping anything, we evolve with the quantum transistor with Jx = $(Jx), Jy = $(Jy), Jz = $(Jz), h = $(h) for $(timeMAX) timeunits.")

times = load("$(deskorlap)/quantum_transistor/after_isingNonInt/L$(L)_Jx$(Jx)_Jy$(Jy)_Jz$(Jz)_h$(h)_Deltat$(Δt)_hZising$(hz_ising)_hXising$(hx_ising)_tTOT$(timeMAX)_BondDim$(bondD)_dt$(dt).jld2", "times")
old_state = load("$(deskorlap)/quantum_transistor/after_isingNonInt/L$(L)_Jx$(Jx)_Jy$(Jy)_Jz$(Jz)_h$(h)_Deltat$(Δt)_hZising$(hz_ising)_hXising$(hx_ising)_tTOT$(timeMAX)_BondDim$(bondD)_dt$(dt).jld2", "t$(round(times[1], digits=3))") 
sites = siteinds(old_state);
...
evolved = TEBD_via_Ising(state_all_up, 1E-12, bondD, dt, Δt);
...
lege = reshape([round(times[i], digits=3) for i in 1:timeSTEPS+1],(1,timeSTEPS+1));

for i in 1:timeSTEPS
    global provisory = TEBD(provisory, 1E-12, bondD, dt, times[i+1]-times[i]);
    ...
    save("$(deskorlap)/quantum_transistor/after_isingNonInt_noflip/L$(L)_Jx$(Jx)_Jy$(Jy)_Jz$(Jz)_h$(h)_Deltat$(Δt)_hZising$(hz_ising)_hXising$(hx_ising)_tTOT$(timeMAX)_BondDim$(bondD)_dt$(dt).jld2"
        , "times", times
        , savingarray...
    )
